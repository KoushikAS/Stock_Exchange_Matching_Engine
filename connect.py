...
def query_order(session: Session, entry: ET.Element, account: Account) -> str:
    ...
    if order_to_query.order_status == OrderStatus.OPEN:
        results.join(f"Order is OPEN with {str(order_to_query.amount)} shares\n")
    else:
        results.join(f"Order is CLOSED with {str(order_to_query.amount)} shares\n")
